...
const Aside = ({ currentUserData,setCurrentUserData }) => {
    const auth = new AppWriteAuth();

    const logOut = async () => {
        try {
            currentUserData && await auth.logOut();
    ...
    return (
        ...
        {currentUserData &&
            <button
                onClick={logOut}
                className="flex justify-center items-center gap-[16px] py-[8px] px-[16px] border-dashed border-2 border-[#DC2626]/60 hover:border-[#DC2626] text-[16px] font-semibold text-[#DC2626]/60 hover:text-[#DC2626] rounded-lg"
            >
                <p>Logout</p>
